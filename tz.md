...
Каждый тип юнита должен иметь уникальную и визуально узнаваемую анимацию атаки
   ...
   **Система смерти и разложения юнитов:**
    ...
    При достижении 0 HP юнит не исчезает мгновенно, а проходит стадии разложения:
     ...
     - **Скелет (20-30 секунд):** Остается только скелет юнита
...
### Расширенная система ворот
#### Система ворот:
**1. улучшения ворот**
- **Назначение:** Базовая защита и замедление врагов
- **Размер:** 1×1 клетка
- **Здоровье:**
  - Уровень 1: 200 HP
  - Уровень 2: 280 HP
...
**1. Горизонтальные ворота**
- **Размер:** 2×1 клетка
- **Назначение:** Основной проход по горизонтали
- **Здоровье:** Соответствует уровню стены 

**2. Вертикальные ворота**
- **Размер:** 1×2 клетки
- **Назначение:** Основной проход по вертикали
- **Здоровье:** Соответствует уровню стены 

**3. Т-образные ворота (буква Т)**
- **Размер:** 3×3 клетки
- **Назначение:** Сложные перекрестки и узлы обороны
- **Здоровье:** Соответствует уровню стены 

**4. Крестообразные ворота**
- **Размер:** 3×3 клетки
- **Назначение:** Центральные узлы обороны
- **Здоровье:** Соответствует уровню стены 
...
2. Происходит анимация перехода (zoom-out)
...
|------|-----|-------------|----------|
| id | INT | PRIMARY KEY, AUTO_INCREMENT | Уникальный идентификатор |
| attacker_id | INT | FOREIGN KEY (users.id) | Атакующий |
| target_village_id | INT | FOREIGN KEY (villages.id) | Целевая деревня |
| army_composition | JSON | NOT NULL | Состав армии |
| start_time | TIMESTAMP | NOT NULL | Время отправки |
