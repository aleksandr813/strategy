...
export default class Building {
    ...
    private static WALL_SPRITES: Record<number, number> = {
        ...
        81: 81, // сверху и снизу или нет соседей
    };


    constructor(id:number, type: string, hp:number, maxHp:number, level:number, size:number, typeId:number, x:number, y:number, wallSpriteIndex?: number) {
        this.id = id;
        this.type = type;
        this.hp = hp;
        this.maxHp = maxHp;
