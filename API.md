...
```
    Answer<{
        ...
        x: number; - координата по x
        y: number; - координата по y
        onACrusade: number; - в походе юнит или нет (возвращается всегда 0 так как юнит вернулся из похода)
    }>
```
**Ошибки**
* `705` - невалидный токен. Пользователь не авторизован
...
### 4.21. unitsAttackDistance
Определяет ближайшего для атаки юнита
...
```
    Answer<{
        attackerId: number; id атакующего юнита
        nearestEnemyUnit: number; id юнита, который должен быть атакован
    }>
```
**Ошибки**
* `705` - невалидный токен. Пользователь не авторизован
...
```
    Answer<{
        user: boolean; есть ли юзер с заданым токеном
    }>
```